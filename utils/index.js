 ...
 * @param atta string
 */
export const fileDownload = atta => {
	var curWwwPath = window.document.location.host;
	console.log('curWwwPath',atta)
	// window.open('http://' + curWwwPath + process.env.VUE_APP_BASE_API + '/schedule/downloadFile?file=' + row.operationSchedulingScheme)
	uni.downloadFile({
	    url: 'http://' + curWwwPath + globalConfig.baseUrl + '/schedule/downloadFile?file=' + atta, //仅为示例，并非真实的资源
	    success: (res) => {
	        console.log(res)
	    }
	});
